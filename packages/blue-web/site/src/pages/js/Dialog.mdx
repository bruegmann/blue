...
```html use:RenderBlock
<button type="button" class="btn blue-btn-soft-secondary" onclick="openControlledTellDialog()">
    Open controlled dialog
</button>

<script>
    async function openControlledTellDialog() {
        setTimeout(() => {
            document.getElementById("myControlledDialog").close()
        }, 3000)
        await blueWeb.dialog.tell("Dialog will close after a few seconds.", { id: "myControlledDialog" })
    }
</script>
```
